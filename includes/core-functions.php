...
function kashiwazaki_seo_llmstxt_get_default_options() {
    ...
    return [
        ...
        'yaml_settings' => [
             ...
             'rate-limit' => ['requests-per-second' => 2, 'requests-per-minute' => 120],
             'canonical-url' => $site_home_url,
             'retry-policy' => ['retry' => true, 'max-retries' => 2, 'retry-after-seconds' => 30, 'status-codes-no-retry' => [ '400', '401', '403', '404' ]],
...
function kashiwazaki_seo_generate_llms_content( $full = false ) {
    ...
    $options = wp_parse_args($saved_options, $default_options);
    ...
    if ($enable_yaml_footer) {
        ...
        if (isset($yaml_settings['rate-limit'])) {
        ...
        if (isset($yaml_settings['retry-policy'])) {
            ...
            if ($retry_enabled) {
                ...
                $retry_after = isset($yaml_settings['retry-policy']['retry-after-seconds']) && is_numeric($yaml_settings['retry-policy']['retry-after-seconds']) ? intval($yaml_settings['retry-policy']['retry-after-seconds']) : $default_options['yaml_settings']['retry-policy']['retry-after-seconds'];
                $temp_yaml_content .= "  max-retries: " . esc_html($max_retries) . "\n";
                $temp_yaml_content .= "  retry-after-seconds: " . esc_html($retry_after) . "\n";
            }
